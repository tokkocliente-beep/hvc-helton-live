...
# o Neon (banco gratuito) suspende o banco por inatividade e/ou fecha
# conexões sem avisar — pool_pre_ping sozinho não bastou (a conexão às vezes
# morre rápido demais pra ele pegar a tempo). NullPool tira o "reaproveitar
# conexão guardada" da equação por completo: cada consulta abre e fecha a
# própria conexão na hora, sem guardar nada pra reusar depois — mais lento
# por consulta, mas nesse app (poucas consultas, plano gratuito) não faz
# diferença perceptível, e elimina esse erro de vez.
...
# ---------------------------------------------------------------------------
# O Neon (banco gratuito) suspende o banco por inatividade — a primeira
# consulta depois de um tempo parado pode cair bem no meio da "acordada" dele
# e falhar com "SSL connection has been closed unexpectedly", mesmo com
# NullPool (que já evita reaproveitar conexão velha). Esse wrapper tenta de
# novo automaticamente (com uma pequena pausa) antes de desistir — na
# prática, a 2ª tentativa quase sempre funciona, porque o banco já acordou.
# ---------------------------------------------------------------------------
from sqlalchemy.exc import OperationalError


def com_retry(func, tentativas=3, espera=1.5):
    for i in range(tentativas):
        try:
            return func()
        except OperationalError as e:
            db.session.rollback()
            print(f"[banco] tentativa {i+1}/{tentativas} falhou: {e}")
            if i == tentativas - 1:
                raise
            time.sleep(espera)
...
def load_user(user_id):
    try:
        return com_retry(lambda: Usuario.query.get(int(user_id)))
    except OperationalError:
        # se nem com retry o banco respondeu, trata como "não logado" em vez
        # de derrubar a página inteira com erro 500
        return None
...
try:
    ...
    # se a primeira tentativa falhar por qualquer motivo (rede, resposta
    # inesperada da IQ Option, etc.), NÃO derruba o servidor inteiro — as
    # próximas chamadas (iq_get_candles_seguro) já tentam reconectar sozinhas
    ...
    with iq_lock:
        try:
            if not iq.check_connect():
                print(f"[candles] iq.check_connect() = False pra {ativo}, reconectando...")
                iq_conectar()
            velas = iq.get_candles(ativo, timeframe, qtd, fim)
            print(f"[candles] get_candles({ativo}, tf={timeframe}, qtd={qtd}) devolveu {len(velas) if velas else 0} velas")
            if velas:
                return velas
        except Exception as e:
            print(f"[candles] EXCEÇÃO em get_candles({ativo}):", repr(e))

        # não veio nada (ou deu exceção) — tenta reconectar e refazer 1 vez
        try:
            reconectou = iq_conectar()
            print(f"[candles] tentativa de reconexão pra {ativo}: {'OK' if reconectou else 'FALHOU'}")
            if reconectou:
                velas2 = iq.get_candles(ativo, timeframe, qtd, fim)
                print(f"[candles] 2ª tentativa devolveu {len(velas2) if velas2 else 0} velas")
                return velas2
        except Exception as e:
            print(f"[candles] EXCEÇÃO na 2ª tentativa pra {ativo}:", repr(e))
        return []
...
def registrar():
    if request.method == "POST":
        ...
        if len(senha) < 6:
            ...
            return redirect(url_for("registrar"))
        if com_retry(lambda: Usuario.query.filter_by(email=email).first()):
...
def login():
    if request.method == "POST":
        ...
        usuario = com_retry(lambda: Usuario.query.filter_by(email=email).first())
...
def candles():
    ...
    print(f"[/candles] pedido recebido: ativo={ativo!r} qtd={qtd}")
    ...
    print(f"[/candles] respondendo com {len(velas)} velas pra ativo={ativo!r}")
